...
def plot_errors(errors_list, output_dir):
    ...
    # Custom tick labels
    labels = [f"{frame}" for frame in frames]  # example: "F123"
    plt.xticks(frames, labels, rotation=45)

    plt.xlabel("Frame index")
    plt.ylabel("MPJPE (px)")
    plt.title("Mean Per Joint Position Error by Frame and Camera")
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel("Frame index")
    plt.ylabel("MSE (px^2)")
    plt.title("Mean Squared Error by Frame and Camera")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
